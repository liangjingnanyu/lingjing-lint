...
(async () => {
  ...
  try {
    if (packageManager === 'yarn') {
      child_process.execSync('yarn config set registry https://registry.npmjs.org/', { stdio: 'inherit' });
    } else {
      child_process.execSync('npm config set registry https://registry.npmjs.org/', { stdio: 'inherit' });
    }
  ...
  try {
    if (packageManager === 'yarn') {
      child_process.execSync(`yarn add inquirer@${inquirerVersion.replace('inquirer@', '')} --dev --ignore-scripts --ignore-engines --no-lockfile --silent`, { stdio: 'inherit' });
    } else {
      child_process.execSync(`npm install inquirer@${inquirerVersion.replace('inquirer@', '')} --no-save --ignore-scripts --legacy-peer-deps --silent`, { stdio: 'inherit' });
    }
  ...
  // 生成 ESLint 配置
  const eslintSource = isTsProject === 'TypeScript'
  ...
  if (confirmFormat === '是的，为我格式化全部代码') {
    try {
      const runFormatCmd = packageManager === 'yarn' ? 'yarn format' : 'npm run format';
      console.log('\n✨ 正在自动格式化代码...');
  ...
  fs.writeJsonSync(vscodeSettingsPath, {
  ...
  console.log('✅ 已生成 .vscode/settings.json，统一团队 TypeScript 类型提示和格式化规则');
