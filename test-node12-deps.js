    ...
    deps.forEach(dep => {
      ...
      if (validateVersionFormat(dep)) {
        const [packageName, version] = dep.includes('@') && !dep.startsWith('@') 
          ? dep.split('@') 
          : [dep.split('@').slice(0, -1).join('@') || dep, dep.split('@').pop()];
...
const allPackages = [
  ...
  "eslint-config-next", "eslint-plugin-vue", "@vue/eslint-config-typescript",
  "@vue/eslint-config-prettier", "eslint-plugin-react-refresh",
